...
//
// Die Entfaltung der Terminregeln steckt in _shared/occurrenceRules.ts und wird
// vom Frontend (src/lib/trainingPlanOccurrences.js) ebenfalls genutzt — es gibt
// sie nur einmal, Kalenderansicht und Abonnement können nicht auseinanderlaufen.
//
...
import { addDaysIso, expandRawOccurrences } from './occurrenceRules.ts'
export { addDaysIso }
...
// [rangeStartIso, rangeEndIso]. Die Entfaltung selbst liegt in
// occurrenceRules.ts; hier wird nur das Feed-Format daraus gebaut.
export function expandSessions(
  ...
  return expandRawOccurrences(sessions, exceptions, rangeStartIso, rangeEndIso).map((o) => {
    const { session, exception } = o
    ...
    return {
      uid: `${session.id}::${o.occurrenceDate}@dolphintennis`,
